...
public class ThingServiceImpl extends ServiceImpl<ThingMapper, Thing> implements ThingService {
    ...
    @Override
    public List<Thing> getThingList(String keyword, String sort, String c, String cc, String ccc) {
        ...
        if (StringUtils.isNotBlank(cc) && !cc.equals("全部")) {
            queryWrapper.eq(true, "pricer", cc);
        }

        // 根据分类3筛选
        if (StringUtils.isNotBlank(ccc) && !ccc.equals("全部")) {
            queryWrapper.eq(true, "mianjir", ccc);
        }
